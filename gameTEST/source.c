...
// Set positions for both eyes (adjust coordinates as needed)
bg->eye_position1 = (SDL_Rect){1242, 516, bg->eye_frames[0]->w, bg->eye_frames[0]->h};
bg->eye_position2 = (SDL_Rect){4678, 516, bg->eye_frames[0]->w, bg->eye_frames[0]->h};
...
bg->eye_frame_delay = 10;
    ...
    if (screen) {
        ...
        for (int i = 0; i < 6; ++i) {
            SDL_Surface* shadow = TTF_RenderText_Solid(bg->font, lines[i], black);
            if (shadow) {
                SDL_Rect pos = {52, 52 + i * 40, 0, 0};
                SDL_BlitSurface(shadow, NULL, screen, &pos);
                SDL_FreeSurface(shadow);
            }
            
            SDL_Surface* text = TTF_RenderText_Solid(bg->font, lines[i], white);
            if (text) {
                SDL_Rect pos = {50, 50 + i * 40, 0, 0};
...
void updateBackground(Background* bg, int scroll_speed) {
    ...
    // Update eye animation only in level 1
    if (bg->current_level == 1) {
        updateEyeAnimation(bg);
    }
...
if (bg->current_level == 1) {
    SDL_Rect eye_pos1 = bg->eye_position1;
    SDL_Rect eye_pos2 = bg->eye_position2;

    if (bg->is_multiplayer) {
        // View 1 - Only render if in view
        if (eye_pos1.x >= bg->view1.camera.x && 
            eye_pos1.x < bg->view1.camera.x + bg->screen_width/2) {
            SDL_Rect render_pos1 = {
                eye_pos1.x - bg->view1.camera.x,
                eye_pos1.y - bg->view1.camera.y,
                eye_pos1.w,
                eye_pos1.h
            };
            SDL_BlitSurface(bg->eye_frames[bg->current_eye_frame], NULL, screen, &render_pos1);
        }
        
        if (eye_pos2.x >= bg->view1.camera.x && 
            eye_pos2.x < bg->view1.camera.x + bg->screen_width/2) {
            SDL_Rect render_pos2 = {
                eye_pos2.x - bg->view1.camera.x,
                eye_pos2.y - bg->view1.camera.y,
                eye_pos2.w,
                eye_pos2.h
            };
            SDL_BlitSurface(bg->eye_frames[bg->current_eye_frame], NULL, screen, &render_pos2);
        }
        
        // View 2 - Only render if in view
        if (eye_pos1.x >= bg->view2.camera.x && 
            eye_pos1.x < bg->view2.camera.x + bg->screen_width/2) {
            SDL_Rect render_pos1 = {
                eye_pos1.x - bg->view2.camera.x + bg->screen_width/2,
                eye_pos1.y - bg->view2.camera.y,
                eye_pos1.w,
                eye_pos1.h
            };
            SDL_BlitSurface(bg->eye_frames[bg->current_eye_frame], NULL, screen, &render_pos1);
        ...
        if (eye_pos2.x >= bg->view2.camera.x && 
            eye_pos2.x < bg->view2.camera.x + bg->screen_width/2) {
            SDL_Rect render_pos2 = {
                eye_pos2.x - bg->view2.camera.x + bg->screen_width/2,
                eye_pos2.y - bg->view2.camera.y,
                eye_pos2.w,
                eye_pos2.h
            };
            SDL_BlitSurface(bg->eye_frames[bg->current_eye_frame], NULL, screen, &render_pos2);
        ...
        // Single player - Only render if in view
        if (eye_pos1.x >= bg->view1.camera.x && 
            eye_pos1.x < bg->view1.camera.x + bg->screen_width) {
            SDL_Rect render_pos1 = {
                eye_pos1.x - bg->view1.camera.x,
                eye_pos1.y - bg->view1.camera.y,
                eye_pos1.w,
        ...
        if (eye_pos2.x >= bg->view1.camera.x && 
            eye_pos2.x < bg->view1.camera.x + bg->screen_width) {
            SDL_Rect render_pos2 = {
                eye_pos2.x - bg->view1.camera.x,
                eye_pos2.y - bg->view1.camera.y,
                eye_pos2.w,
